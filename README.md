...
アーティファクトだらけのボーカルから、クリーンなボーカルを新規生成するツール。

## 目的

録音状態が悪い素材や、Demucs / UVR 等のAI音源分離で得られたボーカルには、分離アーティファクト・帯域欠損・ガビガビノイズが残ることが多い。ITBS-VoGen はこれを入力として、歌い方・節回し（メロディ、フレージング、ビブラート）を保持したままクリーンなボーカルを再生成する。
